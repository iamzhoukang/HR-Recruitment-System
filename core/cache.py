...
from schemas.cache_schema import TaskInfoSchema, InviteInfoSchema, DingTAlkTokenInfoSchema
...
class HRCache(metaclass=SingletonMeta):
    ...
    async def get_invite_info(self,email:str):
        key = f"{self.invite_prefix}{email}"
        invite_info_json = await self.get(key)
        if invite_info_json is not None:
            invite_info_json = InviteInfoSchema.model_validate_json(invite_info_json)
            return invite_info_json
        return None
